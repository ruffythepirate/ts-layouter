import { Polygon, point, Optional, some, none } from 'ts-geometry-visualizer';
    ...
    if (this.layoutShape.isEmpty()) {
      ...
      this.layoutShape.foreach((layoutShape) => {
        ...
        while (layoutShape.overlap(newForm)) {
          ...
          // How do you move one line segment in a direction until it doesn't intercept anymore.
          // Either most distant endpoint needs to be projected onto line, or it might be
          // finished before this, if you project he other lines endpoint onto your line and move.
          // whichever moves the shortest distance. This might create a problem with the overlap,
          // because the line segments might still overlap in a junction.
          // 3. back to start of while.
          newForm = newForm.translate(direction.x, direction.y);
